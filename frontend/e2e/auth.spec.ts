...
test.describe("認証フロー", () => {
  ...
  test("test_login_success", async ({ page }) => {
    ...
    await page.getByLabel(/パスワード/).first().fill(EXISTING_USER.password);
    await page.getByRole("button", { name: /登録/ }).click();
  ...
  test("test_logout_flow", async ({ page }) => {
    ...
    await page.getByLabel(/パスワード/).first().fill(logoutUser.password);
    await page.getByRole("button", { name: /登録/ }).click();
    await page.waitForURL(/\/(login)?/);
  ...
  test("test_login_wrong_password", async ({ page }) => {
    ...
    // エラーメッセージが表示される
    await expect(page.getByText(/パスワード.*間違|認証.*失敗|ログイン.*失敗|Invalid/i)).toBeVisible();
    // ログインページに留まる
    await expect(page).toHaveURL(/\/login/);
  });
  ...
  test("test_login_nonexistent_user", async ({ page }) => {
    ...
    // エラーメッセージが表示される
    await expect(page.getByText(/ユーザー.*存在しない|認証.*失敗|ログイン.*失敗|Invalid/i)).toBeVisible();
  });
  ...
  test("test_register_duplicate_email", async ({ page }) => {
    ...
    await page.getByLabel(/パスワード/).first().fill(dupUser.password);
    await page.getByRole("button", { name: /登録/ }).click();
    await page.waitForURL(/\/(login)?/);
    ...
    // エラーメッセージ
    await expect(page.getByText(/既に.*登録|メール.*使用|already.*registered/i)).toBeVisible();
  });

  test("test_register_invalid_email", async ({ page }) => {
    await page.goto("/register");
    await page.getByLabel(/メールアドレス/).fill("not-an-email");
    await page.getByLabel(/パスワード/).first().fill("TestPass123!");
    await page.getByRole("button", { name: /登録/ }).click();

    // バリデーションエラー
    await expect(page.getByText(/メール.*正しい|有効.*メール|メール.*形式/i)).toBeVisible();
  });
